...
private:
    ...
    std::vector<float> values; // array that holds the last positions, set to 128 by default
    ...
    bool ema_ready = false; // checks to see if its the first ema calculation or not
public:
    explicit ValueSmoothing(int window); // constructor
    void coord_add(float value); // add a coordinate to values array
    bool coord_get_sma(float& out) const; // gets the sma and sets it to the out variable (return true if it can be calculated, false if not)
    bool coord_get_ema(float& out) const; // gets the ema and sets it to the out variable  (return true if it can be calculated, false if not)
    ~ValueSmoothing() = default;
};
