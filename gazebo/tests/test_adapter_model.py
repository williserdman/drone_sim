...
def test_output_epoch_gate_caps_camera_at_configured_final_frame():
    gate = OutputEpochGate(
        expected_frames=1_200,
        public_epoch_native_ns=PUBLIC_EPOCH_NATIVE_NS,
    )
    ...
    assert (
        gate.accept_camera(
            "onboard", PUBLIC_EPOCH_NATIVE_NS + 60_000_000_000
        )
        == 60_000_000_000
    )
    assert (
        gate.accept_camera(
            "onboard", PUBLIC_EPOCH_NATIVE_NS + 60_050_000_000
        )
        is None
    )
...
def test_adapter_tolerates_observed_four_frame_observer_callback_lead():
    adapter = AdapterModel(run_id=RUN_ID, expected_frames=4)
    for stamp_ns in (50_000_000, 100_000_000, 150_000_000, 200_000_000):
        adapter.accept_frame("observer", native_image(stamp_ns=stamp_ns))
...
def test_adapter_tolerates_one_second_private_callback_burst():
