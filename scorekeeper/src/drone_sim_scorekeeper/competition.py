...
class CompetitionScorer:
    ...
    def _phase_events(
        ...
        if len(events) <= index + 1 or not self._mission_prefix(index + 2):
            return None
    ...
    def _landing(self, phase: str, waypoint: str) -> bool:
        ...
        return (
            ...
            and abs(sample.position_xyz[0] - center_x) <= width / 2.0
            and abs(sample.position_xyz[1] - center_y) <= height / 2.0
        )
    ...
    def _delivery(self, marker: int, phase: str) -> tuple[bool, int | None]:
        phase_events = self._phase_events(phase)
    ...
    def _evaluate(self) -> tuple[tuple[bool, ...], bool, str | None]:
        ...
        home_landed = self._landing("HOME", "H")
        elapsed_ns = (
            None
