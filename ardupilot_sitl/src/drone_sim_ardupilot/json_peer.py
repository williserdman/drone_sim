...
class JsonPeer:
    ...
    def exchange_once(self, *, timeout_seconds: float, sim_timestamp: float) -> ServoFrame:
        if not math.isfinite(timeout_seconds) or timeout_seconds <= 0:
            raise ValueError("timeout_seconds must be positive and finite")
        if not math.isfinite(sim_timestamp) or sim_timestamp < 0:
        ...
        if retransmission:
            ...
            response_timestamp = sim_timestamp
            self._last_sim_timestamp = sim_timestamp
