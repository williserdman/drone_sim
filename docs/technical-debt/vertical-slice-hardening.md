...
- Harden the upstream Gazebo plugin's queued-datagram handling after the vertical slice: under severe host contention it can drain multiple sequential actuator packets and retain only the newest, logging `Missed 2 input frames`. Preserve exact-duplicate resend recovery, but process at most one sequential frame per physics update and diagnose a jump beyond `current + 1`. Promote this only if a scored run shows corrupted lockstep evidence rather than an isolated warning.
