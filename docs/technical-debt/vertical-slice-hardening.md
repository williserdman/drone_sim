...
- Resolve the companion MAVLink wall-time discovery bound from run configuration if slow-host startup reaches its current 60-second default before the controller's 120-second startup allowance.
